...
function addWrapper(element, settings) {
    let wrapper = $('<div class="numberedtextarea-wrapper"></div>').insertAfter(element);
...
function addLineNumbers(element, settings) {
    ...
    let wrapper = element.parents('.numberedtextarea-wrapper');

    // Get textarea styles to implement it on line numbers div
    let paddingLeft = parseFloat(element.css('padding-left'));
    let paddingTop = parseFloat(element.css('padding-top'));
    let paddingBottom = parseFloat(element.css('padding-bottom'));

    let lineNumbers = $('<div class="numberedtextarea-line-numbers"></div>').insertAfter(element);
...
function renderLineNumbers(element, settings) {
    ...
    let linesDiv = element.parent().find('.numberedtextarea-line-numbers');
    ...
    for (i = 1; i <= count; i++) {
        let line = $('<div class="numberedtextarea-number numberedtextarea-number-' + i + '">' + i + '</div>').appendTo(linesDiv);
