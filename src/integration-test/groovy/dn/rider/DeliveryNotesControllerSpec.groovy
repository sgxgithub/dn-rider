...
class DeliveryNotesControllerSpec extends Specification {
    ...
    void "testPostAndDeletes"() {
        ...
        def respPostWithWrongRepository = rest.post(url + "/xxx?version=0.0.0-0&repositoryId=ThisIsAWrongRepository") {
            contentType "multipart/form-data"
            dn = '{"NDL_pour_rundeck":{"dependency":[],"packages":[]}}'
        }

        def respPostWithRepositoryNoConformeWithReleaseType = rest.post(url + "/xxx?version=0.0.0-0&repositoryId=asset-releases&releaseType=snapshots") {
            contentType "multipart/form-data"
            dn = '{"NDL_pour_rundeck":{"dependency":[],"packages":[]}}'
        }

        def respPostValid = rest.post(url + "/xxx?version=0.0.0-0&repositoryId=asset-releases") {
            contentType "multipart/form-data"
            dn = '{"NDL_pour_rundeck":{"dependency":[],"packages":[]}}'
        }

        def respPostDuplicated = rest.post(url + "/xxx?version=0.0.0-0&repositoryId=asset-releases") {
            contentType "multipart/form-data"
            dn = '{"NDL_pour_rundeck":{"dependency":[],"packages":[]}}'
        }

        def respPostValidWithReleaseTypeInUrl = rest.post(url + "/xxx/releases?version=0.0.0-1&repositoryId=asset-releases") {
            contentType "multipart/form-data"
            dn = '{"NDL_pour_rundeck":{"dependency":[],"packages":[]}}'
        }

        def respDeleteFirst = rest.delete(url + "xxx/0.0.0-0")
        def respDeleteSecond = rest.delete(url + "xxx/0.0.0-1")
